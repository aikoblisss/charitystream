...
// Register new user - Step 1: Email and Password
app.post('/api/auth/register-step1', async (req, res) => {
  try {
    console.log('📝 Registration Step 1 attempt:', { email: req.body.email });
    ...
    // Create user with verification token (no username yet - will be set in step 2)
    ...
    res.json({
      message: 'Account created successfully! Please choose your username.',
      userId: newUserId,
      email: email,
      nextStep: 'username'
    });
...
// Register new user - Step 2: Username and send verification email
app.post('/api/auth/register-step2', async (req, res) => {
  try {
    console.log('📝 Registration Step 2 attempt:', { userId: req.body.userId, username: req.body.username });
    ...
    const [checkErr, isAvailable] = await dbHelpers.checkUsernameAvailability(username);
    if (checkErr) {
    ...
    // Get user to verify they exist and get verification token
    const [userErr, user] = await dbHelpers.getUserById(userId);
    if (userErr || !user) {
    ...
    if (emailService && emailService.isEmailConfigured()) {
      ...
      const emailResult = await emailService.sendVerificationEmail(user.email, username, user.verification_token);
    ...
    res.json({
      message: 'Username set successfully! Please check your email to verify your account.',
      username: username,
      email: user.email
    });
